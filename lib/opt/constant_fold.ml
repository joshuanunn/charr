...
let fold_unop (op : Ir.unary_operator) (c : Ctype.const) : Ctype.const option =
  ...
  match op with
  | Negate -> Some (Ctype.const_of_int64 (Ctype.const_type c) (Int64.neg n))
  | BwNot -> Some (Ctype.const_of_int64 (Ctype.const_type c) (Int64.lognot n))
  | Not ->
...
let fold_binop (op : Ir.binary_operator) (c1 : Ctype.const) (c2 : Ctype.const) :
    Ctype.const option =
  let n1, n2 = (Ctype.const_to_int64 c1, Ctype.const_to_int64 c2) in
  match op with
  | BwLeftShift ->
      Some
  ...
  | _ -> (
      ...
      match op with
      | Add -> Some (Ctype.const_of_int64 t (Int64.add n1 n2))
      | Subtract -> Some (Ctype.const_of_int64 t (Int64.sub n1 n2))
      | Multiply -> Some (Ctype.const_of_int64 t (Int64.mul n1 n2))
      | Divide ->
          if n2 = 0L then None
          else Some (Ctype.const_of_int64 t (Int64.div n1 n2))
      | Remainder ->
          if n2 = 0L then None
          else Some (Ctype.const_of_int64 t (Int64.rem n1 n2))
      | BwAnd -> Some (Ctype.const_of_int64 t (Int64.logand n1 n2))
      | BwXor -> Some (Ctype.const_of_int64 t (Int64.logxor n1 n2))
