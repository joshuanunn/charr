...
let run_preprocess source = print_string (Pipeline.preprocess source)

let run_lexer source =
  report_errors ~stage:"Lexing" (fun () ->
      let lexbuf = Lexing.from_string (Pipeline.preprocess source) in
      let rec loop () =
        try
...
let run_parser source =
  report_errors ~stage:"Parsing" (fun () ->
      print_endline (Ast.show_prog (Pipeline.parse source)))

let run_validator source s_env t_env =
  report_errors ~stage:"Semantic analysis" (fun () ->
      let ast = Pipeline.validate source s_env t_env in
...
let run_irgen source opts s_env t_env =
  report_errors ~stage:"IR generation" (fun () ->
      print_endline (Ir.show_prog (Pipeline.gen_ir source opts s_env t_env)))

let run_codegen source opts s_env t_env =
  report_errors ~stage:"Code generation" (fun () ->
      let asm, _ = Pipeline.gen_asm source opts s_env t_env in
